...
let kor3 = 23;
// ...
// 동일타입 데잍(들)을 위해 매번 새로운 이름의 변수 생성 불가.
...
arr1 = [10, 20, 30];
arr2 = [10, 20, 30];
console.log(arr1 == arr2);  // JS 에서 object 끼리의 비교는 false
...
// fruits.splice(1, 2, 'Peach');   // 1번째부터 2개 제거, 제거된 위치에 Peach 삽입
fruits.splice(1, 2, 'Peach', '수박', 'Melon'); // 1번째부터 2개 제거, 제거된 위치에 Peach 수박 Melon
console.log(fruits);

fruits.splice(3, 0, '딸기');  // 3번째 위치에 딸기 삽입
console.log(fruits);

fruits.splice(3);     // 3번부터 끝까지 삭제
console.log(fruits);
...
console.log(fruits.concat(['두리앙', '망고']));  // concat 은 원본변화시키지 않습니다
console.log(fruits);

fruits = fruits.concat(['두리앙', '망고']);  // 원본 변화
console.log(fruits);
...
console.log(fruits.includes("두리앙"));  // true
...
arr1[0] = 100;  // array 는 numbered index
arr1[1] = 200;
console.log(arr1);
